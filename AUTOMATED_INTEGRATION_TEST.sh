...
# 로그 파일 생성 안내
echo "테스트 결과를 다음 파일에 기록합니다: $LOG_FILE"
echo "테스트 결과를 다음 파일에 기록합니다: $LOG_FILE" > "$LOG_FILE"
echo "----------------------------------------" | tee -a "$LOG_FILE"
...
error() {
  echo -e "${RED}❌ 오류: ${1}${NC}"
  echo -e "❌ 오류: ${1}" >> "$LOG_FILE"
  echo "전체 테스트 로그는 다음 파일에서 확인할 수 있습니다: $LOG_FILE"
  exit 1
}
...
echo -e "✅ 사용자 계정 관리: 관리자, 일반 사용자, 감사자 계정 생성 및 로그인 성공" >> "$LOG_FILE"

echo -e "${GREEN}✅ 로그인 이벤트 테스트 (수동 승인): 이벤트 생성, 보상 설정, 청구, 관리자 승인 성공${NC}"
echo -e "✅ 로그인 이벤트 테스트 (수동 승인): 이벤트 생성, 보상 설정, 청구, 관리자 승인 성공" >> "$LOG_FILE"

echo -e "${GREEN}✅ 회원가입 이벤트 테스트 (자동 승인): 이벤트 생성, 보상 설정, 청구, 자동 승인 성공${NC}"
echo -e "✅ 회원가입 이벤트 테스트 (자동 승인): 이벤트 생성, 보상 설정, 청구, 자동 승인 성공" >> "$LOG_FILE"

echo -e "${GREEN}✅ 중복 보상 요청 방지: 이미 받은 보상에 대한 중복 요청 거부 확인${NC}"
echo -e "✅ 중복 보상 요청 방지: 이미 받은 보상에 대한 중복 요청 거부 확인" >> "$LOG_FILE"

echo -e "${GREEN}✅ 사용자 권한 제한: 다른 사용자의 이벤트 보상 청구 시도 방지${NC}"
echo -e "✅ 사용자 권한 제한: 다른 사용자의 이벤트 보상 청구 시도 방지" >> "$LOG_FILE"

echo -e "${GREEN}✅ 감사자 테스트: 조회 권한 확인, 수정/승인 권한 제한 확인${NC}"
echo -e "✅ 감사자 테스트: 조회 권한 확인, 수정/승인 권한 제한 확인" >> "$LOG_FILE"

echo -e "\n${BLUE}📝 전체 테스트 로그는 다음 파일에서 확인할 수 있습니다: ${LOG_FILE}${NC}\n" 